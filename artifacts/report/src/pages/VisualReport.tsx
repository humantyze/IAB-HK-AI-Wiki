...
function StatBadge({ point, index }: { point: ChartDataPoint; index: number }) {
  const fills = [
    "bg-[hsl(197,70%,44%)]/15 text-[hsl(197,80%,72%)]",
    "bg-[hsl(245,60%,62%)]/15 text-[hsl(245,70%,80%)]",
    "bg-[hsl(213,55%,52%)]/15 text-[hsl(213,65%,75%)]",
  ];
  const fillClass = fills[index % fills.length];

  return (
    <div className={`rounded-lg px-4 py-3 flex flex-col items-center justify-center min-w-[90px] ${fillClass}`}>
      <span className="text-2xl font-black font-mono tabular-nums leading-none">
        {point.value}{point.unit}
      </span>
      <span className="text-[10px] font-display uppercase tracking-wider mt-1.5 opacity-70 text-center leading-tight max-w-[80px]">
        {point.label}
      </span>
    </div>
  );
  ...
  if (imageUrl) {
    return (
      <div className="relative w-full h-full min-h-[200px]">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-l from-transparent via-transparent to-card/50 hidden lg:block" />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-card/70 lg:hidden" />
      </div>
    );
  }
...
function SectionCard({
  ...
  return (
    ...
    >
      {/* Top accent bar */}
      <div className={`absolute top-0 left-0 right-0 h-[3px] ${accentColor} opacity-80`} />

      {/* Right panel — image */}
      <div className="order-first lg:order-last w-full h-52 lg:h-auto lg:w-[38%] flex-shrink-0 relative overflow-hidden lg:border-l lg:border-border/40">
      ...
      <div className="flex-1 p-8 pt-10 flex flex-col overflow-hidden">
        ...
        <div className="flex items-start justify-between mb-7 relative z-10">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              {section.description && (
                <p className="text-xs text-muted-foreground mt-1.5 font-normal leading-snug max-w-lg">
                  {section.description}
                </p>
              )}
            </div>
          </div>
          <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground/50 font-display tracking-wider uppercase shrink-0 ml-4">
        ...
        {/* Stat Badges */}
        {hasChartData && (
          <div className="flex flex-wrap gap-2.5 mb-7 relative z-10">
        ...
        <div className={`relative z-10 grid gap-6 flex-1 ${hasChartData && chartPoints.length >= 2 ? "lg:grid-cols-2" : "grid-cols-1"}`}>
          {/* Bar Chart */}
          {hasChartData && chartPoints.length >= 2 && (
            <div className="bg-background/40 rounded-xl p-5 border border-border/40">
              <h3 className="font-display font-semibold text-[10px] tracking-[0.2em] uppercase text-primary/80 mb-4 flex items-center gap-2">
          ...
          {section.keyInsights && section.keyInsights.length > 0 && (
            <div className="bg-background/40 rounded-xl p-5 border border-border/40">
              <h3 className="font-display font-semibold text-[10px] tracking-[0.2em] uppercase text-secondary/80 mb-4 flex items-center gap-2">
              ...
              <ul className="space-y-3.5">
                {section.keyInsights.slice(0, 3).map((insight, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <span className="w-[2px] self-stretch rounded-full bg-secondary/50 shrink-0 mt-0.5 min-h-[1.25rem]" />
          ...
          {!hasChartData && (!section.keyInsights || section.keyInsights.length === 0) && (
            <div className="col-span-full flex items-center justify-center h-32 text-muted-foreground/30 text-sm font-display tracking-widest uppercase">
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
          ...
          <div className="flex items-center gap-5">
            <Link href="/">
              <Button
                variant="ghost"
                size="sm"
                className="text-muted-foreground hover:text-foreground font-display uppercase tracking-widest text-xs gap-2 h-8 px-3"
              >
            ...
            <span className="font-serif font-bold text-sm text-foreground/50 tracking-wide">
              State of AI
            </span>
          </div>
          <div className="flex items-center gap-2 bg-primary/10 border border-primary/20 px-3 py-1 rounded-full">
            <span className="w-1.5 h-1.5 rounded-full bg-primary" />
            <span className="font-display text-[10px] uppercase tracking-[0.2em] text-primary/90">
              Visual Summary
            </span>
          </div>
        </div>
      ...
      <section className="relative px-6 pt-14 pb-10 overflow-hidden">
        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.015)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.015)_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_80%_50%_at_50%_0%,#000_60%,transparent_100%)] opacity-30 pointer-events-none" />
        <div className="max-w-[1400px] mx-auto relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2.5 bg-muted/60 border border-border/60 px-3.5 py-1.5 rounded-full text-[10px] font-display tracking-[0.18em] uppercase mb-5 text-muted-foreground">
              <BarChart3 className="w-3 h-3" />
              Data Dashboard · {currentMonthYear}
            </div>
            <h1 className="text-4xl lg:text-5xl font-serif font-black text-foreground/90 leading-tight mb-3">
              State of AI in{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">
                Hong Kong Marketing
              </span>
            </h1>
            <p className="text-muted-foreground text-sm font-normal max-w-2xl leading-relaxed">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between gap-4 pt-2">
              ...
              <Button
                ...
                className="gap-2 font-display uppercase tracking-widest text-xs disabled:opacity-30 disabled:cursor-not-allowed h-8 px-3 border-border/60 hover:border-primary/40 hover:text-primary"
              >
      ...
      <footer className="border-t border-border/50 py-10 px-6 bg-card/20">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <span className="font-serif font-bold text-xl tracking-widest text-foreground/10 select-none">
            STATE OF AI / HK
          </span>
          <Link href="/">
            <Button
              variant="ghost"
              size="sm"
              className="text-muted-foreground hover:text-foreground font-display uppercase tracking-widest text-xs gap-2"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
